...
def get_package_versions(package_name, package_type, owner):
    ...
    while not done:
        response = session.get(f"https://api.github.com/{owner}/packages/{package_type}/{package_name}/versions", params={"per_page" : 100, "page": page})
...
def delete_package_version(package_name, package_type, id, owner):
    ...
    response = session.delete(f"https://api.github.com/{owner}/packages/{package_type}/{package_name}/versions/{id}")
    response.raise_for_status()


def get_tagged_container(package_name, tag, owner):
    print("get_tagged_container", package_name, tag)
    packages = get_package_versions(package_name, "container", owner)
    return [version for version in packages if tag in version["metadata"]["container"]["tags"]]


def delete_tagged_container(package_name, tag, owner):
    print("delete_tagged_container", package_name, tag)
    tags = get_tagged_container(package_name, tag, owner)

    if tags:
        for tag in tags:
            delete_package_version(package_name, "container", tag["id"], owner)
    else:
...
delete_tagged_container(package_name, tag, owner)
